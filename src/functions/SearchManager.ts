...
export class SearchManager {
    ...
    async getSearchPoints(): Promise<SearchPlan> {
        ...
        const doDesktop = this.bot.config.workers.doDesktopSearch && (desktopMissing > 0 || extraDesktopSearches > 0)
        ...
        this.bot.logger.info(
            ...
            `Mobile: ${
                ...
                !this.bot.config.workers.doDesktopSearch
                    ? 'skip (disabled)'
                    : desktopMissing <= 0 && extraDesktopSearches <= 0
                      ? 'skip (no points)'
                      : `run (missing ${desktopMissing}, extra searches ${extraDesktopSearches})`
            }`
        )
