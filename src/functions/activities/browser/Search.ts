...
export class Search extends Workers {
    ...
    public async doSearch(page: Page, isMobile: boolean): Promise<number> {
        ...
        try {
            ...
            if (!isMobile && this.bot.runtimeArgs.extraDesktopSearches > 0) {
                const manualStats = await this.runSearchSession(
                    page,
                    false,
                    new ManualSearchTracker(this.bot, this.bot.runtimeArgs.extraDesktopSearches)
                )
                stats.totalGained += manualStats.totalGained
                stats.performed += manualStats.performed
            }
...
class ManualSearchTracker implements SearchTracker {
    public readonly context = 'SEARCH-BING-MANUAL'
    public readonly stagnantLimit: number
    private performed = 0
    ...
    constructor(
        ...
        this.stagnantLimit = maxSearches + 1
        this.previousBalance = Number(bot.userData.currentPoints ?? 0)
    }

    async prepare(): Promise<boolean> {
        this.bot.logger.info(false, this.context, `Starting ${this.maxSearches} extra desktop searches`)
        return this.maxSearches > 0
    }

    async measure(): Promise<number> {
        this.performed++
        const currentBalance = await this.bot.browser.func.getCurrentPoints()
        const gained = Math.max(0, currentBalance - this.previousBalance)
        this.previousBalance = currentBalance
        this.bot.userData.currentPoints = currentBalance
        if (gained > 0) this.bot.userData.gainedPoints = (this.bot.userData.gainedPoints ?? 0) + gained
        return gained
    }

    done(): boolean {
        return this.performed >= this.maxSearches
    }

    progress(): string {
        return `completed=${this.performed}/${this.maxSearches}`
    }
}
